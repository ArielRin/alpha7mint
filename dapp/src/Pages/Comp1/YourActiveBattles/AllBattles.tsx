...
const calculateTimeLeft = (endTime: Date) => {
  ...
  useEffect(() => {
      ...
      if (window.ethereum) {
        ...
        try {
          ...
          const battlesPromises = battleIds.map(async (id: BigNumber) => {
            ...
            const endTime = new Date(details.startTime.toNumber() * 1000 + (await battleContract.roundDuration()).toNumber() * 1000);
          ...
          setActiveBattles(allBattles); // Correctly use setActiveBattles here
        } catch (error) {
    ...
    fetchBattles();
  }, [setActiveBattles]); // Include setActiveBattles in the dependency array if it's directly referenced or if it might change, though it's typically not necessary for setter functions generated by useState.
