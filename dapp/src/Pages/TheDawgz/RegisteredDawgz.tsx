...
const FetchAllDawgz: React.FC = () => {
  ...
  const fetchNFTData = async () => {
    ...
    try {
        ...
        const nftDataPromises = Array.from({ length: totalSupply.toNumber() }, async (_, i): Promise<NFTItem | null> => {
            const tokenId = i + 1;
            const isRegistered = await dawgRegContract.isNFTRegistered(tokenId);
        ...
        const fetchedNFTs = (await Promise.all(nftDataPromises)).filter((nft): nft is NFTItem => nft !== null);
