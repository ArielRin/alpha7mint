...
setselectedReferrer(contractOptions[key]);
  ...
  return (
    ...
    marginBottom: '10px' // Space below the heading
          ...
          }}>Your Referrer's name</h2>

             <h2 style={{
            color: 'white',
            textAlign: 'center',
            fontSize: '25px', // Example font size, adjust as needed
            fontWeight: 'bold',
            marginBottom: '10px' // Space below the heading
          }}>Rick</h2> */}
  ...
  <select onChange={handleContractChange} style={{ width: '100%', padding: '10px', borderRadius: '4px', border: '1px solid #ccc', color: 'black', backgroundColor: '#f0f0f0' }}>
    {Object.keys(contractOptions).map(option => (
      <option key={option} value={option}>{option}</option>
    ))}
  </select>

  <div style={{ marginBottom: '10px', color: 'white', textAlign: 'left', fontSize: '11px' }}>
    <span style={{ wordBreak: 'break-all' }}>{selectedContract}</span>
      ...
      <a onClick={addTokenToWallet} style={{ display: 'flex', alignItems: 'center', marginTop: '8px', cursor: 'pointer' }}>
        <img src={metamaskLogo} alt="MetaMask" style={{ width: '16px', height: '16px', marginRight: '8px' }} />
        Add ALPHA7 to Wallet
      </a>
