...
const enterBattle = async (tokenId, dawgTaunt) => {
  ...
  const [customTaunt, setCustomTaunt] = useState(null);

  // ... other functions ...

  const enterBattleWithTaunt = () => {
    const tauntToUse = customTaunt || selectedNFTDetails.dawgTaunt;
    enterBattle(selectedNFTDetails.tokenId, tauntToUse);
  };
  ...
  return (
    ...
    <Box mt="20px">
      ...
      >
        {isTransactionLoading ? 'Entering Battle...' : `Enter ${ethers.utils.formatEther(entryFee)} BNB Battle Arena`}
      </Button>
       </Box>
